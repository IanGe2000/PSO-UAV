...
## output format: bool (1 by 1) (if the course conflicts with any threar_source)
    ...
    booltable = ~(~booltable1 & booltable2 & ~booltable3);      ## if each slice conflicts with each threat_source       ## (T by N-1)
...
## output format: objective is a real number
    trajectory = particle2Trajectory (particle, xIntervals);
    if threatConflict([xIntervals; particle]) || ~prod(maxTurningAngle(trajectory, theta_Tmax), "native") || ~prod(maxClimbingDivingAngle(trajectory(1:2,:), theta_Cmax), "native")
        objective = inf;
    else
